...
export default function AddMechanicServicePage() {
  const router = useRouter();
  const params = useParams();
  const vehicleId = params.vehicleId as string;
  
  const { getVehicleById } = useVehicles();
  const { addMechanicService } = useMechanicServices(vehicleId);
  ...
  if (!vehicleName || vehicleName === "Vehículo no encontrado") {
     return (
      <div className="text-center py-10">
        ...
        <p className="text-muted-foreground mb-6">El vehículo para el que intentas agregar un servicio de mecánica no existe.</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-semibold text-center flex items-center justify-center gap-2">
        ...
        Agregar Servicio de Mecánica para <span className="text-primary">{vehicleName}</span>
      </h1>
      <MechanicServiceForm onSubmit={handleSubmit} submitButtonText="Agregar" />
    </div>
  );
}
